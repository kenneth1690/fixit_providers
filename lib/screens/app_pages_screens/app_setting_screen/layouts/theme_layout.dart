...
class ThemeSelectState extends State<ThemeSelect> {
  List themeModeList = [appFonts.darkTheme, appFonts.lightTheme, appFonts.systemDefault];
  ...
  Widget build(BuildContext context) {
    return  Consumer<ThemeService>(builder: (themeContext, theme, child) {
        return Column(
          ...
          ...themeModeList.asMap().entries.map((e) =>
              Column(
                  children: [
                    Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text(language(context, e.value),style: appCss.dmDenseMedium14.textColor(appColor(context).appTheme.darkText)),
