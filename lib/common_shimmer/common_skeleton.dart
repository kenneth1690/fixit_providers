...
class CommonSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Shimmer.fromColors(
      baseColor: baseColor ??( isDark(context)?const Color(0xFF202020).withOpacity(.6): Colors.grey.withOpacity(0.4)),
...
class CommonWhiteShimmer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        decoration: isCircle
            ? BoxDecoration(
                color: isDark(context)?const Color(0xFF202020).withOpacity(.5): appColor(context).appTheme.whiteBg, shape: BoxShape.circle)
            : isSmoothRadius?ShapeDecoration(
            color: appColor(context).appTheme.whiteBg,
            shape: SmoothRectangleBorder(
                borderRadius: SmoothBorderRadius(
                    cornerRadius: radius ?? 8, cornerSmoothing: 1)))  : BoxDecoration(
                color:  isDark(context)?const Color(0xFF202020).withOpacity(.5):appColor(context).appTheme.whiteBg,
